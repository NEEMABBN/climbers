...
export default function Geography({ sectionName, title }) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full border-solid border-Borders border-[1px] rounded-2xl grid grid-cols-3 p-4 gap-y-2">
        {geographyData.map((item, index) => (
          <div
            key={index}
            className="w-full flex gap-1 border-solid border-Borders border-b-[1px]"
          >
            <div className="w-full flex gap-1 border-solid border-Borders border-l-[1px] my-1.5 px-2 py-2">
